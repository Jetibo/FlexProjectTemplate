...
export interface GetWorkerActivitiesResponse {
  success: boolean;
  events: Event[];
}
...
class TaskRouterService extends ApiService {
  ...
  async getWorkerActivityHistory(workerSid: string): Promise<Array<Event>> {
    const response = await this.#getWorkerActivities(workerSid);
    // if (response.success) return response.workerActivities;
    if (response.success) {
      console.log('test getWorkerActivityHistory', response.events);
  ...
  #getWorkerActivities = async (workerSid: string): Promise<GetWorkerActivitiesResponse> => {
    const encodedParams: EncodedParams = {
      workerSid: encodeURIComponent(workerSid),
      Token: encodeURIComponent(this.manager.user.token),
    };

    return this.fetchJsonWithReject<GetWorkerActivitiesResponse>(
      `${this.serverlessProtocol}://${this.serverlessDomain}/common/flex/taskrouter/get-worker-activity-events`,
      {
        method: 'post',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: this.buildBody(encodedParams),
      },
    ).then((response): GetWorkerActivitiesResponse => {
      return response;
    });
  };
